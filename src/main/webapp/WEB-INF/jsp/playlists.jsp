            ...
            <section>
              <section class="vbox">
                <section class="scrollable padder-lg w-f-md" id="bjax-target">
                  <div class="row">
                    <div class="col-md-12">
                      <h3 class="font-bold">My Playlists</h3>
                      <br>
                      <form action="<%=basePath%>playlist/add_playlist.do" method="POST">
                        <div class="form-group">
                          <input name="name" type="text" placeholder="Playlist Name" class="form-control" style="width: 200px; display: inline;">
                          <button type="submit" class="btn btn-s-md btn-success"><i class="fa fa-plus""></i>  Create New Playlist</button>
                        </div>
                      </form>
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td>${loop.index + 1}</td>
                          <td><a href="<%=basePath%>playlist/one_playlist_trans.do?id=${p.pid}">${p.ptitle}</a></td>
                          <td><a href="<%=basePath%>user/profile_trans.do?uid=${p.user.uid}">${p.user.uname}</a></td>
                          <td>${p.pdate}</td>
                          <c:choose>
                            <c:when test="${'1' eq p.pstatus}">
                              <td><i class="icon icon-lock"></i> Private</td>
                              <td class="text-right">
                                <div class="btn-group">
                                  <a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-pencil"></i></a>
                                  <ul class="dropdown-menu pull-right">
                                    <!-- <li><a href="#">Edit Playlist Name</a></li> -->
                                    <li><a href="<%=basePath%>playlist/make_playlist_public.do?id=${p.pid}">Make it Public</a></li>
                                    <li class="divider"></li>
                                    <li><a href="<%=basePath%>playlist/delete_playlist.do?id=${p.pid}">Delete</a></li>
                                  </ul>
                                </div>
                              </td>
                            </c:when>
                            <c:otherwise>
                              <td><i class="fa fa-users"></i> Public</td>
                              <td class="text-right">
                                <div class="btn-group">
                                  <a href="#" class="dropdown-toggle" data-toggle="dropdown"><i class="fa fa-pencil"></i></a>
                                  <ul class="dropdown-menu pull-right">
                                    <!-- <li><a href="#">Edit Playlist Name</a></li> -->
                                    <li><a href="<%=basePath%>playlist/make_playlist_public.do?id=${p.pid}">Make it Private</a></li>
                                    <li class="divider"></li>
                                    <li><a href="<%=basePath%>playlist/delete_playlist.do?id=${p.pid}">Delete</a></li>
                                  </ul>
                                </div>
