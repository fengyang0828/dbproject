            ...
            <section>
              <section class="vbox">
                <section class="scrollable padder-lg w-f-md" id="bjax-target">
                  <h3 class="font-bold">${artist.aname}</h3>
                  <h5>Description: ${artist.adesc}</h5>
                  <h5>Related Artist(s):
                    <c:forEach var="a" items="${artistlist}" varStatus="loop">
                      <c:choose>
                        <c:when test="${'0' eq loop.index}">
                          <a href="<%=basePath%>user/artist_trans.do?aid=${a.aid}">${a.aname}</a>
                        </c:when>
                        <c:otherwise>
                          &nbsp;/&nbsp;<a href="<%=basePath%>user/artist_trans.do?aid=${a.aid}">${a.aname}</a>
                        </c:otherwise>
                    ...
                    <div class="btn-group btn-group-justified m-b">
                      <form role="form" action="<%=basePath%>user/like.do?aid=${artist.aid}" method="POST" style="width: 150px">
                          <c:choose>
                            <c:when test="${'0' eq isFavorite}">
                              <button type="submit" class="btn btn-success btn-rounded"><i class="fa fa-eye"></i> Favorite</button>
                            </c:when>
                            <c:otherwise>
                              <button type="submit" class="btn btn-success btn-rounded"><i class="fa fa-eye"></i> Unfavorite</button>
                            </c:otherwise>
                          </c:choose>
                      </form>
                    </div>
                  ...
                  <div class="row">
                      <div class="col-md-6">
                        ...
                        <tbody>
                          <c:forEach var="t" items="${tracks}" varStatus="loop1">
                          <tr>
                            <td>${loop1.index + 1}</td>
                      ...
                      <div class="col-md-6">
                          ...
                          <tbody>
                            <c:forEach var="a" items="${albums}" varStatus="loop2">
                            <tr>                    
                              <td>${loop2.index + 1}</td>
                              <td><a href="<%=basePath%>album/album_trans.do?alid=${a.alid}">${a.altitle}</a></td>
                              <td><a href="<%=basePath%>user/artist_trans.do?aid=${a.artist.aid}">${a.artist.aname}</a></td>
                              <td>${a.aldate}</td>
                            </tr>
                            </c:forEach>
                          </tbody>
